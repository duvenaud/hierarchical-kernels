...
%	\centerarc[thick] (O1) (\anglemin:\anglemax:\smallradius)
	...
	\draw (left1) node[below = \belowamount] (l1below) {$\embedding(0, 1, \textnormal{true})$};
	
	\draw[fill] (right1) circle (1.5pt);
	\draw (right1) node[below = \belowamount] (r1below) {$\embedding(0, 0, \textnormal{true})$};

	\draw[fill] (O1) circle (1.5pt);
	\draw (O1) node[below = \belowamount] (o1below) {}
	           node[left = 0.3cm] {$\embedding(0, \cdot, \textnormal{false})$};
	...
	\draw (O2) node[below = \belowamount] (o2below) {}
                   node[right] {$\embedding(1, \cdot, \textnormal{false})$};
...
%		\fill[fill=blue!40,fill opacity=0.8](arca1)--(arca2)--(arcb2)--(arcb1)--cycle;
	...
	\draw (right2) node[above] {$\embedding(1, 1, \textnormal{true})$};
