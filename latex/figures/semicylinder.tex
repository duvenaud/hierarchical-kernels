...
\newcommand{\xtwolabelangle}{80}
\newcommand{\belowamount}{0.2cm}
\newcommand{\belowamounttwo}{0.1cm}
...
{ \draw[#1] (#2) ++(#3:#5) arc (#3:#4:#5);
	...
	\draw (left1) node[below = \belowamount, left] {$\embedding(u, \textnormal{true}, 0$)};
	
	\draw[fill] (right1) circle (1.5pt);
	\draw (right1) node[below = \belowamount] (r1below) {};
	\draw (right1) node[left] {$\embedding(\ell, \textnormal{true}, 0)$};

	\draw[fill] (O1) circle (1.5pt);
	\draw (O1) node[below = \belowamount] (o1below) {};
	\draw (O1) node[below] {$\embedding(\cdot, \textnormal{false}, 0)$};
...
%	\draw (left2) node[below, left] {$f(u, \textnormal{true})$};
	...
	\draw (right2) node[right] {$\embedding(\ell, \textnormal{true}, 1)$};
...
%		\draw[green] (arca1) -- (arca2);
	...
	\coordinate(x2half) at ($ (O1)!0.5!(O2) $);
	\draw (x2half) node[below = \belowamounttwo] (x2halfbelow) {$x_1$};
	\draw[trans] ($ (o1below)!0.45!(o2below) $) -- ($ (o1below)!0.3!(o2below) $);
	\draw[trans] ($ (o1below)!0.55!(o2below) $) -- ($ (o1below)!0.7!(o2below) $);

	% Draw arrows showing in which direction x2 varies.
	\coordinate (arclabel) at ($ (O2) + ({\biggerradius*cos(\xtwolabelangle)}, {\biggerradius*sin(\xtwolabelangle)}) $);
	\draw (arclabel) node {$x_2$};
	\centerarc[trans] (O2) ( \xtwolabelangle + 5  :  \xtwolabelangle + 30  :\biggerradius)
	\centerarc[trans] (O2) ( \xtwolabelangle - 5  :  \xtwolabelangle - 30  :\biggerradius)
