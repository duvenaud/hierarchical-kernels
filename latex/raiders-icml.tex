...
\icmltitlerunning{Raiders of the Lost Architecture}

\begin{document}

\twocolumn[
\icmltitle{Raiders of the Lost Architecture:\\Kernels for Bayesian Optimization in Conditional Parameter Spaces}
...
\icmlauthor{Frank Hutter}{fh@informatik.uni-freiburg.de}
\icmladdress{Freiburg University}

\icmlauthor{Michael A. Osborne}{mosb@robots.ox.ac.uk}
\icmladdress{ University of Oxford}
...
$\delta_i\colon \sX\to \{\textnormal{true}, \textnormal{false}\}$,
 for $i \in \{1,\,\ldots,\,D\}$.
  $\delta_i(\vectorize{x})$ stipulates the relevance of the $i$th feature $x_i$ to  
%inference about 
$f(\vectorize{x})$.
...
%\end{align}
%\end{itemize}
%We usually additionally want $k_{\text{FF}}>k_{\text{FT}}$, expressing the fact that points that have identical relevance $\delta_2({\vectorize{x}})$ are more similar than points that differ in relevance $\delta_2({\vectorize{x}})$.
...
%as $f\br_i$. For the definitions, recall that $\delta_i(\vectorize{x})$ is true iff dimension $i$ is relevant given the instantiation of $i$'s ancestors in $\vectorize{x}$.
%
%
\begin{align}
\embeddingletter_i\br(\vectorize{x}) = \left\{\begin{array}{ll}
[0,0]^\transpose & \textrm{ if } \delta_i(\vectorize{x}) = \textrm{ false }\\
...
Figure \ref{fig:cylinder} shows a visualization of the embedding of points $(x_1, \delta_2(\vectorize{x}), x_2)$ into $\reals^3$. 
%
In this space, we have the Euclidean distance,
%
\begin{align}
& d\br_i(\vectorize{x}, \vectorize{x}') = 
||\embeddingletter_i\br(\vectorize{x})-\embeddingletter_i\br(\vectorize{x}')||_2 \nonumber\\
= & \left\{\begin{array}{ll}
0 & \textrm{ if } \delta_i(\vectorize{x}) = \delta_i(\vectorize{x}') = \textrm{false}\\
\omega_i & \textrm{ if } \delta_i(\vectorize{x}) \neq \delta_i(\vectorize{x}')\\
\omega_i \sqrt{2} \sqrt{1 - \cos(\pi\rho_i \frac{x_i-x_i'}{u_i-l_i})} & \textrm{ if } \delta_i(\vectorize{x}) = \delta_i(\vectorize{x}') = \textrm{true}. \end{array}\right.
\label{eq:distance}
\end{align}
...
Explicitly, note that as desired, if $i$ is irrelevant for both $\vectorize{x}$ and $\vectorize{x}'$, $d\br_i$ specifies that $g(\vectorize{x})$ and $g(\vectorize{x}')$ should not differ owing to differences between $x_i$ and $x_i'$. Secondly, if $i$ is relevant for both $\vectorize{x}$ and $\vectorize{x}'$, the difference between $f(\vectorize{x})$ and $f(\vectorize{x}')$ due to $x_i$ and $x_i'$ increases monotonically with increasing $\left|x_i-x_i'\right|$. The parameter $\rho_i$ controls whether differing in the relevance of $i$ contributes more or less to the distance than differing in the value of $x_i$, should $i$ be relevant. 
%
%
%\note{FH: the following 3 sentences are details that could be dropped if we're in need of space.}
%If $\rho = \nicefrac{1}{3}$, and if $i$ is irrelevant for exactly one of $\vectorize{x}$ and $\vectorize{x}'$, $f(\vectorize{x})$ and $f(\vectorize{x}')$ are as different as is possible due to dimension $i$; that is, $f(\vectorize{x})$ and $f(\vectorize{x}')$ are exactly as different in that case as if $x_i=l_i$ and $x_i'=u_i$. For $\rho>\nicefrac{1}{3}$, $i$ being relevant for both $\vectorize{x}$ and $\vectorize{x}'$ means that $f(\vectorize{x})$ and $f(\vectorize{x}')$ could potentially be more different than if %$i$ was relevant in only one of them. For $\rho<\nicefrac{1}{3}$, the converse is true. 
%
%
...
\bibliographystyle{include/icml2014}
